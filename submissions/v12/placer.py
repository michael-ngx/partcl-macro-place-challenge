...
def _eplace_global(
    ...
    verbose: bool = False,
    net_weights: torch.Tensor | None = None,  # NEW: [n_nets] per-net WL multipliers
    init_pos_real: torch.Tensor | None = None,  # NEW: warm-start real cell pos
):
...
class V12Placer:
    ...
    def __init__(
        ...
        gp_iters: int = 600, gp_iters_warm: int = 300,
        n_bins: int = 64, target_density: float = 0.85,
        verbose: bool = False, soft_iters: int = 3, soft_damping: float = 0.5,
        try_init_plc: bool = True,
        n_outer_iters: int = 2, reweight_top_frac: float = 0.10,
    ...
    def place(self, benchmark: Benchmark) -> torch.Tensor:
        ...
        # ─── Iterative DREAMPlace + Reweight + QA loop ────────────
        net_weights = torch.ones(n_nets, device=self.device)
        cur_pos_real = None  # warm-start state for ePlace
        ...
        for it in range(self.n_outer_iters):
            ...
            gp_pos_t, _ = _eplace_global(
                ...
                target_density=self.target_density, verbose=False,
                net_weights=net_weights,
                init_pos_real=cur_pos_real,
            )
            cur_pos_real = gp_pos_t.clone()  # save for next iteration

            gp_full = np.zeros_like(fixed_pos)
            gp_full[:benchmark.num_macros] = gp_pos_t.numpy().astype(np.float64)

            # QA: multi-start legalize + soft jacobi
            ...
            if pos_iter is not None and cost_iter < best_cost:
                best_cost = cost_iter
                best_pos = pos_iter
                best_label = f"iter{it}/{label_iter}"
            if self.verbose:
                print(f"  [iter {it}] gp_iters={iters_this} legal → proxy={cost_iter:.4f}  best={best_cost:.4f}")
            ...
            if pos_iter is not None and self.do_cd_refine and plc is not None:
                ...
                if refined_cost < best_cost:
                    best_cost = refined_cost
                    best_pos = refined
                    best_label = f"iter{it}/CD"
                if self.verbose:
                    print(f"  [iter {it}] gp_iters={iters_this} + CD → proxy={refined_cost:.4f}  best={best_cost:.4f}")

            # REWEIGHT: bump weights on long nets (top K% by HPWL/sqrt(pin_count))
            if it < self.n_outer_iters - 1 and pos_iter is not None:
                ...
                # Decay all weights toward 1 first, then boost top-K
                ...
                if self.verbose:
                    print(f"    reweight: top-{k} nets by hpwl/√n boosted ×{self.reweight_boost}; "
                          f"weights range [{nw_np.min():.2f}, {nw_np.max():.2f}]")

        if best_pos is None:
            best_pos = torch.from_numpy(fixed_pos.copy()).float()

        if self.verbose:
            print(f"  ▶ V12 best: {best_label} → proxy={best_cost:.4f}")

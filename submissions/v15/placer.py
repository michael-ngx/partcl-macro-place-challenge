...
def _real_proxy_cd(
    ...
    """
    Real-proxy CD using IncrementalRealProxy (~1240× faster than
    compute_proxy_cost). Optimizes the EXACT TILOS proxy
    (HPWL + 0.5×density + 0.5×congestion), not a surrogate.

    No revert-on-pass-regression needed since the cost function used
    matches the proxy. Each accepted move strictly improves it.
    """
    # Add v14 dir to path so we can import the sibling module
    import sys, os
    _v14_dir = os.path.dirname(os.path.abspath(__file__))
    if _v14_dir not in sys.path:
        sys.path.insert(0, _v14_dir)
    from incremental_real_proxy import IncrementalRealProxy  # type: ignore
    ...
    # Use real plc as ground truth for "best" (so saved candidate is real-proxy-best)
    ...
    if verbose:
        cur_inc = inc.proxy_cost()
        print(f"    [real-cd] start: plc proxy={initial_real:.4f}, inc proxy={cur_inc:.4f}")
    ...
    for p_idx, step_frac in enumerate(step_fracs[:n_passes]):
        ...
        for i in order:
            ...
            for dx, dy in offsets:
                ...
                if c < best_local[2]:
                    best_local = (nx, ny, c)
            # Optional: try N additional random positions (cheap with fast inc proxy)
        ...
        # Pair-swap pass on the LAST CD pass
        if do_swaps and p_idx == n_passes - 1:
            for i in order:
                ...
                for j in neighbors:
                    ...
                    if new_cost is not None and new_cost < baseline_inc:
                        ...
                        baseline_inc = new_cost

        # Verify with real plc (cheap — once per pass)
        new_pos_t = torch.from_numpy(inc.pos).float()
        new_real = compute_proxy_cost(new_pos_t, benchmark, plc)["proxy_cost"]
        if verbose:
            print(f"    [real-cd] pass {p_idx+1} step={step_frac:.2%}: "
                  f"shifts={improved}/{len(order)} inc={baseline_inc:.4f} plc={new_real:.4f}")
        if new_real < best_real:
            best_real = new_real
            best_pos = new_pos_t.clone()

    # ─── LNS rounds (large neighborhood search) ──────────────
    # Run after CD converges; each round picks K macros and greedy-repairs.
    n_lns_rounds = 15
    n_lns_K = 5
    n_lns_candidates = 24
    lns_accepted = 0
    for r in range(n_lns_rounds):
        if _lns_destroy_repair(
            inc, movable_idx, sep_x, sep_y, half_w, half_h, cw, ch,
            random.Random(rng_seed + 1000 + r),
            K=n_lns_K, n_candidates=n_lns_candidates,
        ):
            lns_accepted += 1
    new_pos_t = torch.from_numpy(inc.pos).float()
    new_real = compute_proxy_cost(new_pos_t, benchmark, plc)["proxy_cost"]
    if verbose:
        print(f"    [lns] {lns_accepted}/{n_lns_rounds} accepted; plc proxy={new_real:.4f}")
    if new_real < best_real:
        best_real = new_real
        best_pos = new_pos_t.clone()
...
def _lns_destroy_repair(
    inc, movable_idx: np.ndarray, sep_x: np.ndarray, sep_y: np.ndarray,
    half_w: np.ndarray, half_h: np.ndarray, cw: float, ch: float,
    rng: random.Random, K: int = 5, n_candidates: int = 32,
) -> bool:
    """
    Large Neighborhood Search round:
      1. Pick K random movable hard macros.
      2. Greedy repair: for each (largest first), try n_candidates random
         positions and pick the lowest-proxy LEGAL position.
      3. If total proxy improves over initial, accept; else revert.

    Returns True if accepted.
    """
    if len(movable_idx) < K:
        K = int(len(movable_idx))
    if K < 1:
        return False

    n_hard = len(half_w)
    cluster = rng.sample(list(movable_idx), K)
    # Repair order: largest area first (more constrained, harder to fit)
    cluster.sort(key=lambda i: -(half_w[i] * half_h[i]))

    initial_cost = inc.proxy_cost()
    saved = {int(i): (float(inc.pos[i, 0]), float(inc.pos[i, 1])) for i in cluster}

    for i in cluster:
        best_x = float(inc.pos[i, 0])
        best_y = float(inc.pos[i, 1])
        best_c = inc.proxy_cost()
        for _ in range(n_candidates):
            nx = rng.uniform(float(half_w[i]), cw - float(half_w[i]))
            ny = rng.uniform(float(half_h[i]), ch - float(half_h[i]))
            ddx = np.abs(nx - inc.pos[:n_hard, 0]); ddy = np.abs(ny - inc.pos[:n_hard, 1])
            mask = (ddx < sep_x[i]) & (ddy < sep_y[i]); mask[i] = False
            if mask.any():
                continue
            c = inc.proposed_move_cost(int(i), nx, ny)
            if c < best_c:
                best_x, best_y, best_c = nx, ny, c
        inc.commit_move(int(i), best_x, best_y)

    final_cost = inc.proxy_cost()
    if final_cost < initial_cost:
        return True
    # Revert
    for i, (sx, sy) in saved.items():
        inc.commit_move(i, sx, sy)
    return False

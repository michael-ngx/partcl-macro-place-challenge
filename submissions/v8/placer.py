"""
v8: v7 + diversified starting layouts (quadratic placement seeding).

v7 trails RePlAce by 0.5% because CD from the initial.plc basin can't
recover on benchmarks where RePlAce found a substantively different
layout. v8 adds a second starting layout — quadratic placement (QP)
of hard macros, which is wire-length-optimal under the convex
relaxation. Pipeline runs full multi-start legalize + soft Jacobi +
fast CD on BOTH initial.plc-warmed AND QP-warmed positions, picks
the lower proxy.

Quadratic placement iterates Gauss-Seidel: for each movable hard
macro, set position to the mean of all connected pin positions
(other macros + ports, weighted by 1/(k-1) per net). Doesn't enforce
overlap freedom (handled by subsequent legalization).
...
def _quadratic_placement(
    ...
    """
    Wire-length-driven quadratic placement of hard macros via Gauss-
    Seidel iteration of the centroid rule. Each iteration: for every
    movable hard macro, set position = mean of all pin positions in
    nets it belongs to (excluding self), weighted by 1/(k-1) per net.

    Soft macros and ports stay where they are; fixed hard macros are
    held fixed; movable hard macros relax toward HPWL-optimal positions.

    No overlap constraint — a subsequent legalization pass (largest-first
    spiral) handles that.

    Args:
        benchmark: source data (net_pin_nodes, port_positions, sizes)
        fixed_pos: [num_macros, 2] starting pos (only soft + fixed-hard
            entries used; movable hard entries are overwritten)
        movable_hard: [n_hard] bool mask of movable hard macros
        cw, ch: canvas dimensions
        n_hard: number of hard macros
        n_iters: Gauss-Seidel sweeps
        damping: 0=no update, 1=full snap to centroid

    Returns:
        [num_macros, 2] new pos with movable hard macros relaxed.
    """
    n_macros = benchmark.num_macros
    n_ports = benchmark.port_positions.shape[0]
    n_owners = n_macros + n_ports
    if len(benchmark.net_pin_nodes) == 0:
        return fixed_pos.copy()
    ...
    pin_offset_xy = owner_pin_offsets[net_owners, net_pinidx]
    port_pos_np = (
        benchmark.port_positions.numpy().astype(np.float64) if n_ports > 0
        else np.zeros((0, 2))
    )

    # Pin entries owned by movable hard macros
    hard_owner_mask = (net_owners < n_hard)
    hard_pin_indices = np.nonzero(hard_owner_mask)[0]
    hard_macro_idx = net_owners[hard_pin_indices]
    hard_pin_net_ids = net_ids[hard_pin_indices]
    ...
    # Initialize movable hard macros at canvas center to start fresh
    cx_init, cy_init = cw / 2, ch / 2
    movable_hard_indices = np.where(movable_hard)[0]
    pos[movable_hard_indices, 0] = cx_init
    pos[movable_hard_indices, 1] = cy_init

    for sweep in range(n_iters):
        owner_pos = np.zeros((n_owners, 2), dtype=np.float64)
        owner_pos[:n_macros] = pos
        if n_ports > 0:
        ...
        # For each pin entry on a hard macro, contribution to its owner's
        # update = (sum_x of net - this pin's x) / (k - 1)
        hpx = pin_xy[hard_pin_indices, 0]
        hpy = pin_xy[hard_pin_indices, 1]
        net_sum_x_h = net_sum_x[hard_pin_net_ids]
        net_sum_y_h = net_sum_y[hard_pin_net_ids]
        net_cnt_h = net_count[hard_pin_net_ids]

        # Avoid div by zero (singleton nets contribute 0)
        contrib_n = np.maximum(net_cnt_h - 1, 0)
        # Weight: 1 contribution per pin entry; aggregated below
        contrib_x = (net_sum_x_h - hpx)
        contrib_y = (net_sum_y_h - hpy)
        # Effective weight per contribution = 1 (each pin contributes once)
        # Counted by contrib_n (number of "neighbors" in this net)
        valid_mask = contrib_n > 0

        sum_x = np.zeros(n_hard, dtype=np.float64)
        sum_y = np.zeros(n_hard, dtype=np.float64)
        sum_n = np.zeros(n_hard, dtype=np.int64)
        np.add.at(sum_x, hard_macro_idx[valid_mask], contrib_x[valid_mask])
        np.add.at(sum_y, hard_macro_idx[valid_mask], contrib_y[valid_mask])
        np.add.at(sum_n, hard_macro_idx[valid_mask], contrib_n[valid_mask])

        new_x = pos[:n_hard, 0].copy()
        new_y = pos[:n_hard, 1].copy()
        update_mask = (sum_n > 0) & movable_hard
        new_x[update_mask] = sum_x[update_mask] / sum_n[update_mask]
        new_y[update_mask] = sum_y[update_mask] / sum_n[update_mask]

        pos_x_hard = pos[:n_hard, 0].copy()
        pos_y_hard = pos[:n_hard, 1].copy()
        pos_x_hard = (1 - damping) * pos_x_hard + damping * new_x
        pos_y_hard = (1 - damping) * pos_y_hard + damping * new_y
        # Clamp to canvas (centers + half size)
        pos_x_hard = np.clip(pos_x_hard, half_w, cw - half_w)
        pos_y_hard = np.clip(pos_y_hard, half_h, ch - half_h)
        # Restore fixed hard macros
        fixed_indices = np.where(~movable_hard)[0]
        pos_x_hard[fixed_indices] = fixed_pos[fixed_indices, 0]
        pos_y_hard[fixed_indices] = fixed_pos[fixed_indices, 1]
        pos[:n_hard, 0] = pos_x_hard
        pos[:n_hard, 1] = pos_y_hard
...
class V8Placer:
    def __init__(
        ...
        do_qp_seed: bool = True,
        qp_iters: int = 30,
        qp_damping: float = 0.7,
        verbose: bool = False,
    ):
        self.seed = seed
    ...
    def place(self, benchmark: Benchmark) -> torch.Tensor:
        ...
        # ─── Stage A: build starting layouts ─────────────────────
        starts = []  # list of (name, pos_np)

        # Start 1: initial.plc (warm)
        starts.append(("init", fixed_pos.copy()))

        # Start 2: quadratic placement seed (cold WL-optimal relaxation)
        if self.do_qp_seed:
            qp_pos = _quadratic_placement(
                benchmark, fixed_pos, movable_hard, cw, ch, n_hard,
                n_iters=self.qp_iters, damping=self.qp_damping,
            )
            starts.append(("qp", qp_pos))

        # ─── Stage B: from each start, run multi-start legalize + soft refine + CD ───
        ...
        for start_name, start_pos in starts:
            # Phase 1: multi-start legalize on this starting layout
            phase_best_cost = float("inf")
            phase_best_pos = None
            for ord_name, order in orderings:
                pos_np = start_pos.copy()
                ...
                if plc is not None:
                    ...
                    if cd["overlap_count"] == 0 and cd["proxy_cost"] < phase_best_cost:
                        phase_best_cost = cd["proxy_cost"]
                        phase_best_pos = full_legal.clone()
                ...
                if plc is not None:
                    ...
                    if cd["overlap_count"] == 0 and cd["proxy_cost"] < phase_best_cost:
                        phase_best_cost = cd["proxy_cost"]
                        phase_best_pos = full_ref.clone()

            if phase_best_pos is None:
                continue

            if self.verbose:
                print(f"  [start={start_name}] multi-start best proxy={phase_best_cost:.4f}")

            # Phase 2: fast CD on top of phase_best
            if plc is not None and self.cd_passes > 0:
                refined_pos, refined_cost = _fast_cd(
                    phase_best_pos, benchmark, plc,
                ...
                if refined_cost < phase_best_cost:
                    phase_best_pos = refined_pos
                    phase_best_cost = refined_cost

            if self.verbose:
                print(f"  [start={start_name}] after CD proxy={phase_best_cost:.4f}")

            if phase_best_cost < best_cost:
                best_cost = phase_best_cost
                best_pos = phase_best_pos.clone()
                best_label = start_name

        if best_pos is None:
            best_pos = torch.from_numpy(fixed_pos.copy()).float()

        if self.verbose:
            print(f"  ▶ V8 best: {best_label} → proxy={best_cost:.4f}")

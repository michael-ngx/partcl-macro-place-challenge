"""
v6: v3 baseline + fast incremental coordinate descent on a custom
proxy surrogate.

The bottleneck in v4 was calling plc.compute_proxy_cost (~70-80 ms)
for every candidate move. v6 maintains its own incremental WL + density
state in NumPy and accepts moves greedily based on a fast surrogate cost.
After each pass it re-validates with the real proxy, accepting the pass
only if the actual proxy improved.
...
def _fast_cd(
    ...
    n_passes: int, step_fracs: tuple, rng_seed: int, verbose: bool = False,
) -> Tuple[torch.Tensor, float]:
    """Fast surrogate-cost CD with periodic real-proxy verification."""
...
class V6Placer:
    def __init__(
        ...
        cd_passes: int = 3,
        cd_step_fracs: tuple = (0.04, 0.02, 0.01),
        verbose: bool = False,
    ):
        self.seed = seed

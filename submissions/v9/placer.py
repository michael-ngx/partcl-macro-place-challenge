"""
v9: v7 + iterated local search (perturb + re-CD).

v7 trails RePlAce by 0.5% on average; it gets stuck in the basin
around initial.plc on several benchmarks. v9 adds an iterated local
search outer loop:

  1. Run v7 (multi-start legalize + soft Jacobi + 5-pass CD) → best L0.
  2. For 3 restarts:
     a. Perturb: pick K random movable hard macros and shift each by
        a Gaussian of σ = 15% canvas. Re-legalize (removes any new
        overlaps). Re-run soft Jacobi.
     b. Re-run a 3-pass CD (finer steps).
     c. Verify with real proxy. If better than current best, keep.
  3. Return best across all restarts + initial.

Each restart explores a different local basin. Time: roughly 1.5×
v7's runtime per benchmark (2 CD calls instead of 1, restarts are
cheap because perturbations are small).
...
def _perturb_and_legalize(
    ...
    """
    Perturb K random movable hard macros by Gaussian shift, then legalize.
    Soft macros are kept untouched (refinement happens in caller via Jacobi).
    """
    pos_np = pos_np.copy()
    movable_idx = np.where(movable_hard)[0]
    canvas_size = max(cw, ch)
    sigma = sigma_frac * canvas_size

    # Pick K random macros to perturb
    if n_perturb > len(movable_idx):
        n_perturb = len(movable_idx)
    perturb_set = rng.sample(list(movable_idx), n_perturb)

    half_w = sizes_np[:n_hard, 0] / 2
    half_h = sizes_np[:n_hard, 1] / 2

    for i in perturb_set:
        pos_np[i, 0] = float(np.clip(pos_np[i, 0] + rng.gauss(0, sigma),
                                      half_w[i], cw - half_w[i]))
        pos_np[i, 1] = float(np.clip(pos_np[i, 1] + rng.gauss(0, sigma),
                                      half_h[i], ch - half_h[i]))

    # Re-legalize with area-desc ordering
    area = sizes_np[:n_hard, 0] * sizes_np[:n_hard, 1]
...
class V9Placer:
    def __init__(
        ...
        # ILS parameters
        n_restarts: int = 3,
        restart_perturb_frac: float = 0.30,  # K = frac * n_movable
        restart_sigma_frac: float = 0.15,
        restart_cd_passes: int = 3,
        restart_cd_step_fracs: tuple = (0.04, 0.02, 0.01),
        verbose: bool = False,
    ):
        self.seed = seed
        ...
        self.restart_perturb_frac = restart_perturb_frac
        self.restart_sigma_frac = restart_sigma_frac
        self.restart_cd_passes = restart_cd_passes
        self.restart_cd_step_fracs = restart_cd_step_fracs
    ...
    def place(self, benchmark: Benchmark) -> torch.Tensor:
        ...
        # ─── Stage 2: full fast CD with swaps ─────────────────────────
        ...
        if self.verbose:
            print(f"  ▶ After main CD: proxy={best_cost:.4f}")

        # ─── Stage 3: iterated local search (perturb + re-CD) ────────
        if plc is not None and self.n_restarts > 0:
            n_movable = int(movable_hard.sum())
            n_perturb = max(1, int(self.restart_perturb_frac * n_movable))
            for r in range(self.n_restarts):
                rng = random.Random(self.seed + r + 1)
                pos_np = best_pos.numpy().astype(np.float64).copy()
                # Perturb K random macros
                pos_perturbed = _perturb_and_legalize(
                    pos_np, sizes_np, movable_hard, fixed_pos, cw, ch, n_hard,
                    rng, n_perturb=n_perturb, sigma_frac=self.restart_sigma_frac,
                )
                # Soft jacobi refine
                pos_perturbed = _soft_jacobi_update(
                    pos_perturbed, benchmark, n_iters=self.soft_iters,
                    damping=self.soft_damping,
                )
                full = torch.from_numpy(pos_perturbed).float()
                # Verify
                cd = compute_proxy_cost(full, benchmark, plc)
                if cd["overlap_count"] != 0:
                    continue
                # CD again with shorter schedule
                refined_pos, refined_cost = _fast_cd(
                    full, benchmark, plc,
                    n_passes=self.restart_cd_passes,
                    step_fracs=self.restart_cd_step_fracs,
                    rng_seed=self.seed + r + 1,
                    do_swaps=self.do_swaps,
                    n_swap_neighbors=self.n_swap_neighbors,
                    verbose=False,
                )
                if self.verbose:
                    print(f"    [restart {r+1}] perturbed→{cd['proxy_cost']:.4f} → CD→{refined_cost:.4f}")
                if refined_cost < best_cost:
                    best_cost = refined_cost
                    best_pos = refined_pos.clone()
                    if self.verbose:
                        print(f"    [restart {r+1}] NEW BEST: {best_cost:.4f}")

        if self.verbose:
            print(f"  ▶ V9 final best: {best_cost:.4f}")

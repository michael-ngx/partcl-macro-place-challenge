...
# ───────────── DREAMPlace optional swap (set DREAMPLACE_DIR env var) ─────── #
# When DREAMPLACE_DIR is set, _eplace_global uses real DREAMPlace via the
# Bookshelf bridge in dreamplace_wrapper.py. Otherwise it uses the pure-PyTorch
# ePlace reimplementation below.
_DREAMPLACE_DIR = os.environ.get("DREAMPLACE_DIR")
_REAL_DP_FN = None
if _DREAMPLACE_DIR:
    try:
        # Add v16 dir to path so we can import the sibling module
        _v16_dir = os.path.dirname(os.path.abspath(__file__))
        if _v16_dir not in sys.path:
            sys.path.insert(0, _v16_dir)
        from dreamplace_wrapper import (  # type: ignore
            DreamPlaceWrapper, make_eplace_global_fn,
        )
        _DP = DreamPlaceWrapper(_DREAMPLACE_DIR, use_gpu=True)
        _REAL_DP_FN = make_eplace_global_fn(_DP)
        print(f"[v16] Using REAL DREAMPlace from {_DREAMPLACE_DIR}")
    except Exception as e:
        print(f"[v16] Failed to load real DREAMPlace ({e}); falling back "
              f"to pure-PyTorch ePlace.")
        _REAL_DP_FN = None


# ───────────────────── ePlace global placement ──────────────── #
def _eplace_global_pytorch(
    benchmark: Benchmark,
    device: torch.device,
    n_iters: int = 600,
    n_bins: int = 64,
...
# ─────────────── ePlace dispatcher (real DREAMPlace if available) ────────── #
def _eplace_global(*args, **kwargs):
    """Drop-in `_eplace_global` that uses real DREAMPlace if `DREAMPLACE_DIR`
    env var is set and the wrapper imports succeeded; else falls back to the
    pure-PyTorch reimplementation `_eplace_global_pytorch`.

    Signature matches `_eplace_global_pytorch` exactly so callers don't change.
    """
    if _REAL_DP_FN is not None:
        return _REAL_DP_FN(*args, **kwargs)
    return _eplace_global_pytorch(*args, **kwargs)

"""
v10: v7 + congestion (RUDY) in the surrogate.

v7 trails RePlAce by 0.5%; the surrogate optimizes only WL + density
while ~64% of the actual proxy is congestion. CD finds the local
optimum of the WL+density landscape, which doesn't necessarily align
with proxy optima.

v10 adds a RUDY-style net-routing-demand estimator to the surrogate:
  H_demand(g) = Σ_{e ∈ E} (overlap_area(BBOX(e), g) / (W_e × H_e)) × W_e
  V_demand(g) = Σ_{e ∈ E} (overlap_area(BBOX(e), g) / (W_e × H_e)) × H_e
The congestion surrogate is the average of the top 5% of
max(H_demand, V_demand) values across grid cells.

IncrementalProxy maintains per-net bounding boxes and updates the
RUDY grid in O(bbox_cells_per_affected_net) on each move. Surrogate
cost = wl + 0.5·density + 0.5·rudy_cong, matching the proxy weights.

Below is the original v7 doc for reference.

OLD v7 NOTES: v6 + more passes + pair-swap moves to escape local minima.
...
class IncrementalProxy:
    """
    Maintains incremental HPWL, density grid, and RUDY-style routing
    congestion grid for fast move evaluation.

    Surrogate cost = wl_weight * (total_HPWL / n_nets / (cw+ch))
                   + den_weight * mean(top_10pct(density_grid))
                   + cong_weight * mean(top_5pct(rudy_grid))

    First two terms match the proxy's wirelength_cost and density_cost.
    The RUDY term is a simplified routing-demand estimator that
    correlates with the proxy's congestion_cost (which uses K-net
    decomposition; we use bounding-box-based RUDY here for speed).
    """
    ...
    def __init__(self, benchmark: Benchmark, full_pos: np.ndarray,
        ...
        # Per-net bounding boxes and RUDY grid
        self.net_bbox = np.zeros((n_nets, 4), dtype=np.float64)  # x_lo, x_hi, y_lo, y_hi
        self.rudy_grid = np.zeros((self.gr, self.gc), dtype=np.float64)
        self._recompute_bbox_and_rudy_full()
    ...
    def _recompute_bbox_and_rudy_full(self):
        """Compute per-net bounding box and full RUDY grid."""
        self.rudy_grid.fill(0.0)
        for nid in range(self.n_nets):
            entries = self.net_to_pin_entries[nid]
            if len(entries) <= 1:
                self.net_bbox[nid] = 0.0
                continue
            xs = self.pin_xy[entries, 0]
            ys = self.pin_xy[entries, 1]
            self.net_bbox[nid, 0] = xs.min()
            self.net_bbox[nid, 1] = xs.max()
            self.net_bbox[nid, 2] = ys.min()
            self.net_bbox[nid, 3] = ys.max()
            self._add_net_rudy_contribution(nid, +1.0)

    def _add_net_rudy_contribution(self, nid: int, sign: float):
        """
        Add or subtract net nid's RUDY contribution to the rudy_grid based on
        its current self.net_bbox entry. RUDY contribution to cell g:
            rudy_g = (overlap_area(g, bbox) / (W * H)) * (W + H)
                  = overlap_area(g, bbox) * (1/H + 1/W)
        """
        x_lo, x_hi, y_lo, y_hi = self.net_bbox[nid]
        W = x_hi - x_lo
        H = y_hi - y_lo
        if W <= 0 or H <= 0:
            return
        # Bell-shape factor: 1/W + 1/H (inverse demand width)
        density_factor = 1.0 / W + 1.0 / H
        # Bin range
        x_lo_c = max(x_lo, 0.0); x_hi_c = min(x_hi, self.cw)
        y_lo_c = max(y_lo, 0.0); y_hi_c = min(y_hi, self.ch)
        if x_hi_c <= x_lo_c or y_hi_c <= y_lo_c:
            return
        col_lo = int(x_lo_c / self.bin_w); col_hi = int(min((x_hi_c - 1e-12) / self.bin_w, self.gc - 1))
        row_lo = int(y_lo_c / self.bin_h); row_hi = int(min((y_hi_c - 1e-12) / self.bin_h, self.gr - 1))
        col_lo = max(0, min(col_lo, self.gc - 1)); col_hi = max(0, min(col_hi, self.gc - 1))
        row_lo = max(0, min(row_lo, self.gr - 1)); row_hi = max(0, min(row_hi, self.gr - 1))
        ...
        for r in range(row_lo, row_hi + 1):
            ...
            for c in range(col_lo, col_hi + 1):
                ...
                ox = max(0.0, min(x_hi, rx_hi) - max(x_lo, rx_lo))
                if ox <= 0: continue
                self.rudy_grid[r, c] += sign * ox * oy * density_factor
    ...
    def cong_cost(self) -> float:
        """Surrogate congestion: top 5% mean of rudy_grid."""
        flat = self.rudy_grid.flatten()
        n_top = max(1, int(np.ceil(len(flat) * 0.05)))
        top_vals = np.partition(flat, -n_top)[-n_top:]
        return float(top_vals.mean())

    def surrogate_cost(self) -> float:
        wl = self.total_hpwl() / max(self.n_nets * (self.cw + self.ch), 1e-12)
        return (self.wl_weight * wl
                + self.den_weight * self.density_cost()
                + self.cong_weight * self.cong_cost())
    ...
    def proposed_move_cost(self, macro_idx: int, new_x: float, new_y: float) -> float:
        ...
        old_bbox_for_nets = {nid: self.net_bbox[nid].copy() for nid in affected_nets}
        new_hpwl_for_nets = {}
        new_bbox_for_nets = {}
        ...
        for nid in affected_nets:
            ...
            if len(entries) <= 1:
                ...
                new_bbox_for_nets[nid] = np.zeros(4)
                continue
            xs = self.pin_xy[entries, 0]
            ys = self.pin_xy[entries, 1]
            new_hpwl_for_nets[nid] = (xs.max() - xs.min()) + (ys.max() - ys.min())
            new_bbox_for_nets[nid] = np.array([xs.min(), xs.max(), ys.min(), ys.max()])
        ...
        # ── RUDY delta: subtract old, add new for affected nets ────
        for nid in affected_nets:
            self._add_net_rudy_contribution(int(nid), -1.0)
            self.net_bbox[nid] = new_bbox_for_nets[nid]
            self._add_net_rudy_contribution(int(nid), +1.0)
        new_cong_cost = self.cong_cost()

        # Restore RUDY
        for nid in affected_nets:
            self._add_net_rudy_contribution(int(nid), -1.0)
            self.net_bbox[nid] = old_bbox_for_nets[nid]
            self._add_net_rudy_contribution(int(nid), +1.0)
        ...
        return (self.wl_weight * wl_norm
                + self.den_weight * new_density_cost
                + self.cong_weight * new_cong_cost)
    ...
    def commit_move(self, macro_idx: int, new_x: float, new_y: float):
        ...
        for entry in self.owner_to_pin_entries[macro_idx]:
            self.pin_xy[entry, 0] = new_x + self.pin_offset_xy[entry, 0]
            self.pin_xy[entry, 1] = new_y + self.pin_offset_xy[entry, 1]
        # Per-net HPWL + RUDY refresh for affected nets only
        affected_nets = np.unique(self.net_ids[self.owner_to_pin_entries[macro_idx]])
        for nid in affected_nets:
            entries = self.net_to_pin_entries[nid]
            # Subtract old RUDY for this net
            self._add_net_rudy_contribution(int(nid), -1.0)
            if len(entries) <= 1:
                self.net_hpwl[nid] = 0.0
                self.net_bbox[nid] = 0.0
